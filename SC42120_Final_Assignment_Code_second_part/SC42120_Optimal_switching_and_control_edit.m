...
Q1_bar = t_switch*dt*Q;                              % Discretized Q1        %DONE                
Q2_bar = t_switch*dt*Q;                              % Discretized Q2        %DONE
R1_bar = t_switch*dt*R;                              % Discretized R1        %DONE
R2_bar = t_switch*dt*R;                              % Discretized R2        %DONE
f_1_bar = @(x1,x2)([x1;x2]+t_switch*dt*f_1(x1,x2)); % Discretized f_1       %DONE
f_2_bar = @(x1,x2)([x1;x2]+t_switch*dt*f_2(x1,x2));  % Discretized f_2       %DONE
g_1_bar = @(x1,x2)(t_switch*dt*g_1);                 % Discretized g_1       %DONE
g_2_bar = @(x1,x2)(t_switch*dt*g_2);                 % Discretized g_2       %DONE
...
for t = 0:N-1 % time goes from 0 to tf ----> N number of sample 
    ...
    if diverged == 0
        ...
        if k == N % Step 1: least square to initialize the final critic weight W
            ...
            FinalW(:,k) = pinv(LHS_J' * LHS_J) * LHS_J' * RHS_J; %DONE
            ...
            for i=1:MaxEpochNo-1
                for j = 1:NoOfEquations 
                    ...
                    % Step 6
                    if k*dt < t_bar 
                        X_k_plus_1 = f_1_bar(X_k(1),X_k(2)) + g_1_bar(X_k(1),X_k(2))*temp_U_k; %DONE
                        U_k = -(R1_bar)^-1 * (g_1_bar(X_k(1),X_k(2)))'*dphi_dx(X_k(1),X_k(2),t_switch)'*FinalW(:,k+1);%TODO
                    else
                        X_k_plus_1 = f_2_bar(X_k(1),X_k(2)) + g_2_bar(X_k(1),X_k(2))*temp_U_k; %DONE
                        U_k = -(R2_bar)^-1 * (g_2_bar(X_k(1),X_k(2)))'*dphi_dx(X_k(1),X_k(2),t_switch)'*FinalW(:,k+1);%TODO
                    end
                    RHS_U(j,:) = U_k';
                ...
                % Step 7
                Vi_1(:,k,i+1) = pinv(LHS_U' * LHS_U) * LHS_U' * RHS_U;%DONE
